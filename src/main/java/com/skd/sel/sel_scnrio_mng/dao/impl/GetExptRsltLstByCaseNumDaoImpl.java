...
/**
 * 이 클래스는 셀레니움 테스트 플랫폼 시스템에서 시나리오 번호 및 케이스 번호가 있을 때 그에 해당하는 예상 기대 결과 리스트를 가져오기 위한
 * 클래스이다.<br/>
 ...
 * @version 1.0
 */
@Repository("getExptRsltLstByCaseNumDao")
public class GetExptRsltLstByCaseNumDaoImpl implements GetExptRsltLstByCaseNumDao {
  private static Logger logger = LogManager.getLogger(GetExptRsltLstByCaseNumDaoImpl.class);

  @Autowired
  private SqlSession sqlSession;
  /**
   * 입력받은 시나리오 번호 및 케이스번호를 기준으로 테스트 예상 결과 기준 리스트를 되돌려준다.
   *
   ...
   *        rslt_strd : 결과 기준
   *        judg_typ_cd : 판정 유형 코드
   *        judg_typ_nm : 판정 유형 명
   * @exception SQLException SQL 수행 시 발생할 수 있는 예외
   */
  public List<Map<String, Object>> getExptRsltLstByCaseNum(Map<String, Object> inputMap) throws SQLException {
    logger.debug("============   Start method of GetExptRsltLstByCaseNumDaoImpl.getExptRsltLstByCaseNum   ============");
    logger.debug("Parameter inputMap[" + inputMap.toString() + "]");
    List<Map<String, Object>> outputList = null;
    outputList = sqlSession.selectList("com.skd.sel.sel_scnrio_mng.getExptRsltLstByCaseNum", inputMap);
    logger.debug("Output Map[" + outputList.toString() + "]");
    logger.debug("============   End method of GetExptRsltLstByCaseNumDaoImpl.getExptRsltLstByCaseNum   ============");
    return outputList;    
  }
}